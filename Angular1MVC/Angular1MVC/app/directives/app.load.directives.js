...
mvcapp.directive('fileuploadForm', ['$upload', function ($upload) {
    return {
        scope: {
            asyncurl: '@',
            successCallback: '&',
...
mvcapp.directive('dropfile', function () {
    return {
        scope: {
            browse: '&',
            config: '='
...
mvcapp.directive('progressWidth', [function () {
    return {
        restrict: 'A',
        scope: {
            progressWidth: '='
...
mvcapp.directive('sideNavbar', ['$location', 'authService', function ($location, authService) {
	return {
        ...
        link: function (scope, element, attr, cntrl) {
            ...
            scope.$on('updatelogin', function (evt) {
                scope.loggeduser = authService.getAuthCode();
                scope.$apply();
            });
